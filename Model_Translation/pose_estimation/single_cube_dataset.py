...
class SingleCubeDataset(torch.utils.data.IterableDataset):
    ...
    def pre_processing(self, element_iterator):
        ...
        Returns:
            a tuple of four element which are:
                - an image tensor
                - a tensor vector for the 3 cube's center
                coordinates (translation)
                - the tensor vector for the 4 quaternion elements
                (orientation)
                - the tensor vector of size 3 for the Y axis scale element
                  of each cube
        """
        ...
        for i in range(len(position_list)):
            obj_height = position_list[i]["size"]["y"]
            if obj_height>max_height:
                max_height = obj_height
                max_height_ind = i
            #translation.extend(list(position_list[i]["translation"].values()))
            #orientation.extend(list(position_list[i]["rotation"].values()))
            #scaleY.append(position_list[i]["size"]["y"])
            #classes.append(name2class[position_list[i]["label_name"].split("_")[0]])    
        if max_height_ind ==-1:
            translation = [-1]*3
            scaleY = [-1]*1
        else:
            translation = list(position_list[max_height_ind]["translation"].values())
            scaleY = [position_list[max_height_ind]["size"]["y"]]
        ...
        image_Depth = Image.open(image_name).convert("RGBA").getchannel("A")
        imageDepth = transform(image_Depth).unsqueeze(0)

        return imageRGB, imageDepth, translation, scaleY
