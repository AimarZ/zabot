...
def evaluate_one_epoch(*, estimator, config, data_loader, epoch, test):
    ...
    with torch.no_grad():
        metric_translation, metric_orientation, metric_scaleY = evaluation_over_batch(
            estimator=estimator,
            config=config,
...
def evaluation_over_batch(
    *,
    ...
    for index, (images, target_translation_list, target_orientation_list, target_scaleY_list) in enumerate(
        data_loader
    ):
        ...
        if estimator.model.is_symetric == False:
            output_translation, output_orientation, output_scaleY = estimator.model(
            ...
            if is_training:
                ...
                train_loss = (
                    loss_translation + config.train.beta_loss * loss_orientation + loss_scaleY
                )
    ...
    return metric_translation, metric_orientation, metric_scaleY
